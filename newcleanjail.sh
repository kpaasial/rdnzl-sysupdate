...
# We need this to know which branch we are installing to the new portsjail.
...
if test "${OBJ_SVNREVISION}" -lt "${SRC_SVNREVISION}"; then
    echo "OBJ_SVNREVISION ${OBJ_SVNREVISION} is lesser than SRC_SVNREVISION ${SRC_SVNREVISION}."
...
# dismantling first) on the filesystem this script will not know that,
